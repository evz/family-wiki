...
class ResearchQuestionGenerator:
    ...
    def load_data(self) -> None:
        ...
        if self.llm_results_file.exists():
            with open(self.llm_results_file, 'r', encoding='utf-8') as f:
                data = json.load(f)
    ...
    def analyze_naming_patterns(self) -> List[ResearchQuestion]:
        ...
        main_surnames = [name for name, count in surname_counts.most_common(5)]
    ...
    def analyze_historical_context(self) -> List[ResearchQuestion]:
        ...
        if 'wwii' in affected_periods:
            questions.append(ResearchQuestion(
                ...
                evidence=f"Family records extend into WWII period",
                priority="high",
                research_type="archival",
...
JSON:"""
    ...
    def save_questions(self, output_file: str = "research_questions.json") -> None:
        ...
        questions_data = {
            'metadata': {
                ...
                'categories': list(set(q.category for q in self.research_questions))
            },
            'questions': [
                {
                    'category': q.category,
    ...
    def print_summary(self) -> None:
        ...
        print(f"\n{'='*80}")
        print(f"RESEARCH QUESTIONS FOR VAN BULHUIS FAMILY HISTORY")
        print(f"{'='*80}")
        ...
        print(f"\n🎯 NEXT STEPS:")
        print(f"1. Review research_questions.json for complete list")
        print(f"2. Start with high-priority, easy-difficulty questions")
        print(f"3. Focus on archival research for missing vital records")
        print(f"4. Explore historical context for broader understanding")
