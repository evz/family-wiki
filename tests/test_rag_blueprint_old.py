...
class TestRAGBlueprint:
    ...
    @pytest.fixture
    def mock_embedding_models(self):
    ...
    def test_submit_query_with_corpus_selection(self, client, db, mock_rag_service_class):
        """Test submitting query with corpus selection"""

        # Create a completed corpus
        corpus = TextCorpus(
            name="Test Corpus",
        ...
        with patch('web_app.blueprints.rag._get_or_create_session') as mock_get_session:
            ...
            mock_rag_instance.generate_rag_response.assert_called_once()

    def test_submit_query_missing_corpus_id(self, client, mock_rag_service_class):
        """Test submitting query without corpus selection"""
        response = client.post('/rag/submit-query', data={
            'question': 'Who was born in Amsterdam?',
            'session_name': 'Test Session'
        })
    ...
    def test_submit_query_corpus_not_found(self, client, db):
        """Test submitting query with nonexistent corpus"""
        fake_corpus_id = str(uuid4())
    ...
    def test_submit_query_corpus_not_ready(self, client, db):
        """Test submitting query with corpus that's not ready"""

        # Create a corpus that's not ready
        corpus = TextCorpus(
    ...
    def test_create_corpus_get(self, client, mock_embedding_models):
        """Test GET request to create corpus page"""
        response = client.get('/rag/corpora/create')

        assert response.status_code == 200
        assert b'Create Text Corpus' in response.data
        assert b'multilingual-e5-large' in response.data

    def test_create_corpus_post_success(self, client, db, mock_rag_service_class, mock_embedding_models):
        """Test successful corpus creation"""
        # Mock corpus creation
        mock_corpus = Mock()
        mock_corpus.id = str(uuid4())
        mock_corpus.name = "Test Corpus"

        mock_rag_instance = mock_rag_service_class.return_value
        mock_rag_instance.create_corpus.return_value = mock_corpus

        with patch('web_app.blueprints.rag.safe_task_submit') as mock_task_submit:
            with patch('web_app.blueprints.rag.validate_embedding_model') as mock_validate:
                mock_validate.return_value = True  # Mock validation success
                mock_task = Mock()
                mock_task.id = "task-123"
                mock_task_submit.return_value = mock_task

                # Create test file content using proper file-like object
                from io import BytesIO
                file_data = BytesIO(b"This is test corpus content about Dutch genealogy.")

                response = client.post('/rag/corpora/create',
                    data={
                        'name': 'Test Corpus',
                        'description': 'Test corpus for genealogy',
                        'embedding_model': 'zylonai/multilingual-e5-large',
                        'text_file': (file_data, 'test.txt')
                    },
                ...
                mock_task_submit.assert_called_once()
    ...
    def test_submit_query_success(self, db, client, mock_rag_service_class):
        """Test successful query submission with corpus selection"""
        ...
        with patch('web_app.blueprints.rag._get_or_create_session') as mock_get_session:
            ...
            # Submit query with corpus selection
            response = client.post('/rag/submit-query', data={
