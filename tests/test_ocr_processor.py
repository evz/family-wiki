...
class TestPDFOCRProcessor:
    ...
    @patch('web_app.pdf_processing.ocr_processor.argparse.ArgumentParser')
    def test_main_single_pdf_exists(self, mock_argparse, mock_processor_class):
        """Test main function with single PDF file that exists"""
        # Mock argument parser
        mock_parser = Mock()
        mock_args = Mock()
        mock_args.single_pdf = "test.pdf"
        ...
        # Mock processor
        mock_processor = Mock()
        mock_processor.output_dir = Path("output")
        mock_processor.process_pdf.return_value = "Extracted text"
        mock_processor_class.return_value = mock_processor

        # Mock Path.exists
        with patch('web_app.pdf_processing.ocr_processor.Path.exists', return_value=True), \
             patch('builtins.open', mock_open()), \
             patch('builtins.print') as mock_print:

            from web_app.pdf_processing.ocr_processor import main
            main()
    ...
    @patch('web_app.pdf_processing.ocr_processor.PDFOCRProcessor')
    @patch('web_app.pdf_processing.ocr_processor.argparse.ArgumentParser')
    def test_main_single_pdf_not_exists(self, mock_argparse, mock_processor_class):
        """Test main function with single PDF file that doesn't exist"""
        # Mock argument parser
        mock_parser = Mock()
        mock_args = Mock()
        mock_args.single_pdf = "nonexistent.pdf"
        mock_args.output_dir = "output"
        mock_parser.parse_args.return_value = mock_args
        mock_argparse.return_value = mock_parser
        ...
        with patch('web_app.pdf_processing.ocr_processor.Path.exists', return_value=False), \
             patch('builtins.print') as mock_print:

            from web_app.pdf_processing.ocr_processor import main
            main()

            mock_processor.process_pdf.assert_not_called()
            mock_print.assert_called_once_with("PDF file not found: nonexistent.pdf")

    @patch('web_app.pdf_processing.ocr_processor.PDFOCRProcessor')
    @patch('web_app.pdf_processing.ocr_processor.argparse.ArgumentParser')
    def test_main_process_all_pdfs_dir_exists(self, mock_argparse, mock_processor_class):
        """Test main function processing all PDFs when directory exists"""
        # Mock argument parser
        mock_parser = Mock()
        mock_args = Mock()
        mock_args.single_pdf = None
        mock_args.pdf_dir = "pdfs"
        mock_args.output_dir = "output"
        mock_parser.parse_args.return_value = mock_args
        mock_argparse.return_value = mock_parser
    ...
    @patch('web_app.pdf_processing.ocr_processor.PDFOCRProcessor')
    @patch('web_app.pdf_processing.ocr_processor.argparse.ArgumentParser')
    def test_main_process_all_pdfs_dir_not_exists(self, mock_argparse, mock_processor_class):
        """Test main function when PDF directory doesn't exist"""
        # Mock argument parser
        mock_parser = Mock()
        mock_args = Mock()
        mock_args.single_pdf = None
        mock_args.pdf_dir = "nonexistent_dir"
        mock_args.output_dir = "output"
        mock_parser.parse_args.return_value = mock_args
        mock_argparse.return_value = mock_parser
        ...
        with patch('web_app.pdf_processing.ocr_processor.Path.exists', return_value=False), \
            ...
            mock_processor.process_all_pdfs.assert_not_called()
            mock_print.assert_called_once_with("PDF directory not found: nonexistent_dir")

    @patch('web_app.pdf_processing.ocr_processor.argparse.ArgumentParser')
    def test_main_argument_parsing(self, mock_argparse):
        ...
        mock_args.single_pdf = None
        mock_args.pdf_dir = "pdfs"
        mock_args.output_dir = "extracted_text"
        mock_parser.parse_args.return_value = mock_args
        mock_argparse.return_value = mock_parser
        ...
        with patch('web_app.pdf_processing.ocr_processor.Path.exists', return_value=False), \
            ...
            mock_parser.add_argument.assert_any_call("--pdf-dir", default="pdfs", help="Directory containing PDF files")
            mock_parser.add_argument.assert_any_call("--output-dir", default="extracted_text", help="Output directory for text files")
            mock_parser.add_argument.assert_any_call("--single-pdf", help="Process a single PDF file")
