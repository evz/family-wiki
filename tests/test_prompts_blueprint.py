...
class TestPromptsBlueprint:
    ...
    def test_list_prompts_success(self, client, app, mock_prompt_service, mock_prompt):
        ...
        # Should be called twice: once for extraction prompts, once for RAG prompts
        assert mock_prompt_service.get_all_prompts.call_count == 2
        actual_calls = [call.kwargs['prompt_type'] for call in mock_prompt_service.get_all_prompts.call_args_list]
        assert 'extraction' in actual_calls
        assert 'rag' in actual_calls

    def test_list_prompts_no_prompts(self, client, app, mock_prompt_service):
        """Test prompts listing with no prompts"""
        # Setup mocks
    ...
    def test_save_prompt_create_success(self, client, app, mock_prompt_service):
        ...
        mock_prompt_service.create_prompt.assert_called_once_with(
            'New Prompt', 'Test prompt text', 'extraction', 'Test description'
        )
    ...
    def test_save_prompt_whitespace_trimming(self, client, app, mock_prompt_service):
        ...
        assert response.status_code == 302
        mock_prompt_service.create_prompt.assert_called_once_with(
            'Test Prompt', 'Test prompt text', 'extraction', 'Test description'
        )
    ...
    def test_save_prompt_optional_description(self, client, app, mock_prompt_service):
        ...
        assert response.status_code == 302
        mock_prompt_service.create_prompt.assert_called_once_with(
            'Test Prompt', 'Test prompt text', 'extraction', ''
        )
