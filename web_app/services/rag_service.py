        ...
        try:
            ...
            if isinstance(corpus_id, str):
                try:
                    ...
                    raise ValueError(f"Corpus not found: {corpus_id}") from None
            ...
            self.logger.error(f"Error storing source text: {e}")
            db.session.rollback()
            raise
    ...
    def load_pdf_text_files(self, corpus_id: str) -> dict:
        ...
        try:
            ...
            if not text_dir.exists():
                return {'success': False, 'error': 'Extracted text directory not found'}
            ...
            for txt_file in text_dir.glob("*.txt"):
                ...
                try:
                    ...
                    self.logger.error(f"Error processing {txt_file}: {e}")
                    continue
            ...
            self.logger.error(f"Error loading PDF text files: {e}")
...
ANSWER:"""
            ...
            if response.status_code == 200:
                ...
                query.status = 'failed'
                query.error_message = f"LLM request failed: {response.status_code}"
